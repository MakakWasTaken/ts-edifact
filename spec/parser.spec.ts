...
describe('Parser', () => {
    describe('initialize', () => {
        it('should accept known encodings', () => {
            expect(
                () => new Parser(new Configuration({ charset: 'UNOA' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOB' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOC' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOD' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOE' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOF' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOG' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOH' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOI' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOJ' }))
            ).not.toThrow();
            expect(
                () => new Parser(new Configuration({ charset: 'UNOK' }))
            ).not.toThrow();
        });

        it('should reject unknown encodings', () => {
            expect(
                () => new Parser(new Configuration({ charset: 'UNKNOWN' }))
            ).toThrow();
        });
    });
    ...
    describe('write', () => {
        ...
        beforeEach(() => {
            parser = new Parser(
                new Configuration({ validator: new NullValidator() })
            );
        });
        ...
        it('should accept sample message with two parsers & same encodings (config is cached)', () => {
            const msg =
                "UNB+UNOC:3+123:14+123:14+180813:0806+404114++LG'UNH+404114+MSCONS:D:04B:UN:2.1a'";
