...
export abstract class Charset implements RegExType {
    ...
    constructor(
        ...
        admissibleAlphabet: string[][],
        unicode: boolean = false
    ) {
        this.name = name;
        const exclude: number[] = configuration.delimiters();
        const alphas: RegExp[] = this.compile(
            admissibleAlphabet,
            exclude,
            unicode
        );
                ...
                if (start && end) {
                    for (let i: number = start; i <= end; i++) {
                        if (!excludes.includes(i)) {
                            output += Separators.escapeIfNeeded(
...
export class Tokenizer {
    ...
    setCharsetBasedOnConfig(config: Configuration): RegExType {
        switch (config.charset) {
            ...
            default:
                throw new Error(
                    `Unsupported charset encoding '${config.charset}'`
                );
        }
        return this.regexes;
    }
    ...
    length(): number {
        return (
            this.buffer.length - (this.regex === this.regexes.decimal ? 1 : 0)
        );
    }
